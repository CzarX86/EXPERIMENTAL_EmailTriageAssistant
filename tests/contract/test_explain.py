...
def test_explain_returns_details():
    ...
    try:
        resp = requests.get(f"{BASE_URL}/explain/{email_id}", headers=headers, timeout=2)
    except Exception:
        assert False, "API server not running"
    assert resp.status_code == 200
    data = resp.json()
    for key in ("email_id", "label", "confidence", "key_factors", "similar_emails", "feedback_influence"):
        assert key in data
